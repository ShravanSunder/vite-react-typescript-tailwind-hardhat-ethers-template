...
const publishDir = '../vite-app-ts/src/contracts';
const graphDir = '../subgraph';

function publishContract(contractName: string) {
  console.log(' 💽 Publishing', chalk.cyan(contractName), 'to', chalk.gray(publishDir));
  try {
    let contractString = fs.readFileSync(`${hre.config.paths.artifacts}/contracts/${contractName}.sol/${contractName}.json`).toString();
